...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='192.168.4.1'" $ZZZ                              # IPv4 地址(openwrt后台地址)
sed -i "/uci commit network/i\uci set network.lan.netmask='255.255.255.0'" $ZZZ                           # IPv4 子网掩码
sed -i "/uci commit network/i\uci set network.lan.gateway='192.168.4.1'" $ZZZ                             # IPv4 网关
...
echo " Compiled in $(TZ=UTC-8 date "+%Y.%m.%d")" > package/base-files/files/etc/openwrt_gxqm     # 增加个性名字281677160
